...
function App() {
  ...
  const loginWithGoogle = useGoogleLogin({
    ...
    // FIX: Request explicit permission structures, offline renewal access, and prompt consent for long-term token maintenance.
  ...
  const syncEventToGoogleCalendar = async ( targetEvent ) => {
    ...
    try {
      // FIX: Standardize startTime and endTime formatting into true ISO-8601 strings
      const isoStart = new Date(`${targetEvent.date}T${targetEvent.startTime || '09:00'}:00`).toISOString();
      const isoEnd = new Date(`${targetEvent.date}T${targetEvent.endTime || '17:00'}:00`).toISOString();

      const gcalEvent = {
        summary: targetEvent.title,
        location: targetEvent.location,
        description: `${targetEvent.content || ""}\n\nOrganized by: ${targetEvent.organizer}`,
        start: { dateTime: isoStart, timeZone: 'Asia/Kuala_Lumpur' },
        end: { dateTime: isoEnd, timeZone: 'Asia/Kuala_Lumpur' }
      };
