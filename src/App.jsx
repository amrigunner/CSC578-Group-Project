  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-fadeIn">
      <div className="w-full max-w-md rounded-3xl bg-white p-6 text-center shadow-2xl border border-gray-100 flex flex-col items-center space-y-4">
        ...
        <h3 className="text-sm font-black text-purple-950 uppercase tracking-wide">{title}</h3>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans text-gray-800">
      <div className="bg-purple-950 text-white text-center py-2.5 text-xs font-medium tracking-wide">
        Event Management with Notification
      </div>
      
      <header className="bg-white border-b border-gray-100 relative pt-6 pb-4">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center relative">
          <div className="flex flex-col items-center text-center cursor-pointer mb-5" onClick={() => setCurrentView('home')}>
            <img src={uitmLogo} alt="UiTM Official Crest" className="h-16 w-auto object-contain mb-2" />
          </div>
          
          <div className="absolute right-4 top-4 text-xs z-20 flex items-center gap-3">
            <button onClick={() => { setIsNotifOpen(true); markAllAsRead(); }} className="relative p-2 text-gray-600 hover:text-purple-950 hover:bg-gray-100 rounded-full transition-all cursor-pointer" title="Notification Log Feed">
            ...
            {userProfile ? (
              <div className="flex items-center gap-2">
                <div className="bg-purple-950 text-white font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                  <span className="text-[10px] capitalize">[{userRole}] {userProfile.name.split(' ')[0]}</span>
                </div>
                {userRole === 'user' && (
                  <button onClick={() => setCurrentView('submit_event')} className="bg-amber-500 hover:bg-amber-600 text-white px-2.5 py-1.5 rounded-lg font-bold transition-all cursor-pointer text-[10px]"> Submit Event</button>
                )}
                <button onClick={handleLogout} className="bg-red-600 hover:bg-red-700 text-white px-2.5 py-1.5 rounded-lg font-bold transition-all cursor-pointer text-[10px]">Logout</button>
              </div>
            ) : (
              <button onClick={() => loginWithGoogle()} className="bg-purple-950 hover:bg-black text-white px-4 py-2 rounded-xl font-bold transition-all cursor-pointer shadow-md tracking-wide">
      ...
      {/* Main Body view splitting handles layout structures natively */}
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-6">
        {/* VIEW: SUBMIT NEW PROPOSAL */}
        {currentView === 'submit_event' && (
          <div className="max-w-2xl mx-auto bg-white border rounded-3xl p-6 shadow-sm">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              <h2 className="text-sm font-black text-purple-950 uppercase tracking-wider">Propose New Campus Event</h2>
              <button onClick={() => setCurrentView('home')} className="text-gray-400 font-bold text-xs">Back</button>
            </div>
            <form onSubmit={handleEventFormSubmit} className="space-y-4 text-xs">
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" onClick={() => setCurrentView('home')} className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl cursor-pointer">Cancel</button>
                <button type="submit" className="px-6 py-2.5 bg-purple-950 hover:bg-black text-white font-bold rounded-xl cursor-pointer transition-all">Submit to Review Pipeline</button>
              </div>
            </form>
          </div>
        )}
        ...
        {(currentView === 'home' || currentView === 'admin_dashboard') && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-1.5 border-b pb-4 border-gray-100">
              <span className="text-[11px] uppercase font-black text-purple-950 mr-2 tracking-wider">Classification filter:</span>
            ...
            {userRole === 'admin' && currentView === 'admin_dashboard' && (
              <div className="space-y-6">
                <div className="bg-purple-50 rounded-3xl p-4 flex flex-wrap gap-2 border border-purple-100 shadow-xs">
                  <button onClick={() => setAdminSubView('analytics')} className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${adminSubView === 'analytics' ? 'bg-purple-950 text-white' : 'text-purple-950 hover:bg-purple-100'}`}>📈 System Operations Overview</button>
                  <button onClick={() => setAdminSubView('proposals_queue')} className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer relative ${adminSubView === 'proposals_queue' ? 'bg-purple-950 text-white' : 'text-purple-950 hover:bg-purple-100'}`}>
                  ...
                  <button onClick={() => setAdminSubView('broadcast_man')} className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${adminSubView === 'broadcast_man' ? 'bg-purple-950 text-white' : 'text-purple-950 hover:bg-purple-100'}`}>📢 Broadcast Notice Board Writer</button>
                </div>
                          ...
                          <div key={req.id} className="bg-white border rounded-3xl p-4 flex flex-col justify-between shadow-xs">
                            <div className="space-y-2">
                              <div className="flex justify-between items-start">
                                <span className="bg-amber-100 text-amber-800 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">{req.category || 'Event'}</span>
                                <span className="text-gray-400 text-[10px] font-mono">ID: {req.id}</span>
                              </div>
                              <h4 className="font-black text-purple-950 text-sm leading-tight">{req.title}</h4>
                              <p className="text-[11px] text-gray-600 line-clamp-3 leading-relaxed">{req.content}</p>
                              <div className="text-[10px] text-gray-500 grid grid-cols-2 gap-1 pt-1 font-medium bg-gray-50 p-2 rounded-xl">
                                <div>📅 Date: {formatToDisplayDate(req.date)}</div>
                                <div>📍 Venue: {req.location}</div>
                                <div>🕒 Time: {req.startTime} - {req.endTime}</div>
                                <div>👥 Group: {req.organizer}</div>
                              </div>
                            </div>
                            <div className="flex gap-2 justify-end pt-3 mt-3 border-t border-gray-100">
                              <button onClick={(e) => handleRejectRequest(req.id, e)} className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 font-bold rounded-lg text-[11px] cursor-pointer transition-all">Decline Submission</button>
                              <button onClick={(e) => handleApproveRequest(req.id, e)} className="px-4 py-1.5 bg-purple-950 hover:bg-black text-white font-bold rounded-lg text-[11px] cursor-pointer transition-all shadow-xs">Clear & Approve Live</button>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {adminSubView === 'broadcast_man' && (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-1 bg-white border rounded-3xl p-5 shadow-sm">
                      {editingAnnId ? (
                        <form onSubmit={saveUpdatedAnn} className="space-y-3 text-xs">
                          <div className="flex justify-between items-center border-b pb-1.5 mb-2">
                            <h4 className="font-black text-purple-950 uppercase tracking-wider">Modify Notice Row</h4>
                            <button type="button" onClick={() => setEditingAnnId(null)} className="text-gray-400 font-bold">Cancel</button>
                          </div>
                          <div>
                    ...
                    <div className="lg:col-span-2 space-y-3">
                      ...
                      <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                        {announcements.map(ann => (
                          <div key={ann.id} className="bg-white border rounded-2xl p-3 flex justify-between items-start gap-4 hover:shadow-xs transition-all text-xs border-gray-100">
                            <div className="space-y-1 flex-1">
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded font-black text-[9px] uppercase ${ann.category === 'Urgent' ? 'bg-red-100 text-red-700 animate-pulse' : 'bg-purple-100 text-purple-800'}`}>{ann.category || 'General'}</span>
                                <span className="text-gray-400 font-medium text-[10px]">{formatToDisplayDate(ann.date)}</span>
                              </div>
                              <h5 className="font-bold text-purple-950">{ann.title}</h5>
                              <p className="text-[11px] text-gray-600 line-clamp-2 leading-relaxed">{ann.content}</p>
                            </div>
            ...
            {(currentView === 'home' || currentView === 'announcements' || currentView === 'categories' || currentView === 'admin_dashboard') && (
              <div className="space-y-4">
                <h2 className="text-sm font-black uppercase text-purple-950 tracking-wider flex items-center gap-2 border-b pb-1.5 border-gray-100">
                ...
                {filteredAnnouncements.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {filteredAnnouncements.map(ann => (
                      <div key={ann.id} onClick={() => setActiveDetails({ ...ann, type: 'announcement' })} className="bg-white border border-gray-100 rounded-3xl p-5 shadow-xs hover:shadow-md transition-all cursor-pointer flex flex-col justify-between space-y-4 group relative">
                        <div className="space-y-2">
                          <div className="flex justify-between items-center text-[10px] text-gray-400 font-medium">
                            <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wide ${ann.category === 'Urgent' ? 'bg-red-500 text-white animate-pulse' : 'bg-gray-100 text-gray-700'}`}>{ann.category || 'General'}</span>
                            <span>{formatToDisplayDate(ann.date)}</span>
                          </div>
                          <h3 className="font-black text-purple-950 text-sm group-hover:text-purple-700 transition-colors leading-tight line-clamp-2">{ann.title}</h3>
                        ...
                        {userRole === 'admin' && (
                          <div className="flex justify-end gap-2 pt-2 border-t border-gray-50 mt-1" onClick={(e) => e.stopPropagation()}>
            ...
            {(currentView === 'home' || currentView === 'events' || currentView === 'admin_dashboard') && (
              <div className="space-y-4 pt-4">
                <div className="flex justify-between items-center border-b pb-1.5 border-gray-100">
                  <h2 className="text-sm font-black uppercase text-purple-950 tracking-wider flex items-center gap-2">
                    <span>🗓️</span> Upcoming Live Campus Schedules {selectedCategory !== 'All' && `(${selectedCategory})`}
                  </h2>
                  {userRole === 'admin' && (
                    <div className="bg-white border rounded-2xl p-1 flex gap-1">
                      <button onClick={() => setEditingEventId(null)} className="px-3 py-1 bg-purple-50 text-purple-950 font-bold rounded-xl text-[10px] uppercase">Active Matrix</button>
                    </div>
                  )}
                </div>

                {editingEventId ? (
                  <div className="bg-purple-50/50 border rounded-3xl p-5 max-w-xl animate-fadeIn">
                    <form onSubmit={saveUpdatedEvent} className="space-y-3 text-xs">
                      <div className="flex justify-between items-center border-b pb-2 mb-2">
                        <h4 className="font-black text-purple-950 uppercase tracking-wide">✏️ Update Live Event Parameters</h4>
                        <button type="button" onClick={() => setEditingEventId(null)} className="text-gray-400 font-bold">Cancel</button>
                      </div>
                ...
                {filteredEvents.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredEvents.map(event => (
                      <div key={event.id} onClick={() => setActiveDetails({ ...event, type: 'event' })} className="bg-white border rounded-3xl overflow-hidden shadow-xs hover:shadow-xl transition-all cursor-pointer flex flex-col justify-between group relative border-gray-100">
                        {userRole === 'admin' && (
                        ...
                        <div>
                          ...
                          <div className="p-5 space-y-2">
                            <div className="flex justify-between items-center text-[10px] font-bold text-amber-600 uppercase tracking-wide">
                              <span>{event.category || 'Event'}</span>
                              <span className="text-gray-400 font-mono text-[9px]">#{event.id}</span>
                            </div>
                            <h3 className="font-black text-purple-950 text-base leading-snug group-hover:text-purple-700 transition-colors line-clamp-2">{event.title}</h3>
                            <p className="text-xs text-gray-600 line-clamp-2 leading-relaxed">{event.content || "No extended synopsis supplied for this operational segment node."}</p>
                          </div>
                        </div>
                        <div className="p-5 pt-0">
                          <div className="bg-gray-50 p-3 rounded-2xl text-[11px] text-gray-600 font-medium space-y-1">
      ...
      {activeDetails && (
        <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn" onClick={() => { if(!showRegisterForm) setActiveDetails(null); }}>
          <div className="bg-white rounded-3xl w-full max-w-xl max-h-[85vh] overflow-y-auto shadow-2xl p-6 space-y-5 animate-scaleUp" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start border-b pb-3 border-gray-100">
              <div className="space-y-1">
                <span className="bg-purple-100 text-purple-800 text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full">{activeDetails.category || activeDetails.type || 'Details'}</span>
                <h2 className="text-base font-black text-purple-950 leading-tight pt-1">{activeDetails.title}</h2>
              </div>
              <button onClick={() => { setActiveDetails(null); setShowRegisterForm(false); }} className="text-gray-400 hover:text-black font-black text-sm p-1 cursor-pointer">✕</button>
            </div>

            {activeDetails.image && (
              <div className="w-full h-48 rounded-2xl overflow-hidden bg-gray-50 border shadow-inner">
                <img src={activeDetails.image} alt="Modal presentation graphic attachment" className="w-full h-full object-cover" />
              </div>
            )}

            <div className="text-xs text-gray-700 space-y-3 leading-relaxed whitespace-pre-wrap max-h-[250px] overflow-y-auto pr-1">
              {activeDetails.content}
            </div>

            {(activeDetails.type === 'event' || activeDetails.type === 'announcement') && (
              <div className="bg-purple-50 p-4 rounded-2xl text-xs text-purple-950 space-y-2 border border-purple-100/50">
                ...
                <div className="grid grid-cols-2 gap-2 text-gray-700 font-medium">
                  <div>📅 Scheduled Date: {formatToDisplayDate(activeDetails.date)}</div>
                  {activeDetails.type === 'event' && (
                    <>
                      <div>📍 Exact Venue: {activeDetails.location}</div>
                      <div>🕒 Time Interval: {activeDetails.startTime} - {activeDetails.endTime}</div>
                      <div>👥 Lead Host Team: {activeDetails.organizer}</div>
                    </>
                  )}
                </div>
              </div>
            ...
            {activeDetails.type === 'event' && userRole === 'user' && (
              <div className="pt-2 border-t border-gray-100">
                {!showRegisterForm ? (
                  <button onClick={() => setShowRegisterForm(true)} className="w-full py-3 bg-purple-950 hover:bg-black text-white font-black rounded-xl text-xs uppercase tracking-wider transition-all cursor-pointer shadow-md">
                    Reserve & Register Seat Securely
                  </button>
                ) : (
                  <form onSubmit={handleStudentRegistration} className="space-y-3 bg-gray-50 p-4 rounded-2xl border text-xs animate-fadeIn">
                    ...
                    <div>
                      ...
                      <input
                        type="email" required placeholder="username@student.uitm.edu.my" className="w-full p-2 bg-white border rounded-xl"
                    ...
                    <div className="flex gap-2 justify-end pt-2 font-bold">
                      ...
                      <button
                        ...
                        className="px-5 py-2 bg-purple-950 text-white rounded-lg hover:bg-black transition-all cursor-pointer shadow-xs disabled:opacity-50"
                      >
            ...
            <div className="space-y-4 flex-1 flex flex-col overflow-hidden">
              <div className="flex justify-between items-center border-b pb-3">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-black uppercase text-purple-950 tracking-wider">System Pipeline Log</span>
                  {unreadCount > 0 && <span className="bg-red-500 text-white font-bold text-[9px] px-1.5 py-0.5 rounded-full animate-pulse">{unreadCount} New</span>}
                </div>
                <button onClick={() => setIsNotifOpen(false)} className="text-gray-400 hover:text-black font-bold text-sm p-1 cursor-pointer">✕</button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-1 text-xs">
                {notifications.length === 0 ? (
                  ...
                  notifications.map(notif => (
                    <div key={notif.id} className={`p-3 rounded-xl border transition-all relative ${notif.unread ? 'bg-purple-50/60 border-purple-100 shadow-xs' : 'bg-white border-gray-100'}`}>
                      ...
                      <p className="text-[11px] text-gray-600 mt-0.5 leading-normal">{notif.description}</p>
      ...
      <footer className="bg-purple-950 text-white text-xs mt-auto border-t-4 border-amber-400">
        <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            <h4 className="font-black uppercase text-amber-400">UiTM Events Manager Cluster</h4>
            <p className="text-purple-200 text-[11px] leading-relaxed max-w-xs">An enterprise real-time event pipeline solution bridging direct Supabase clusters with student Google Workspaces natively.</p>
          </div>
          <div className="space-y-1">
        ...
        <div className="bg-purple-900/40 text-center py-3 text-[10px] text-purple-300 border-t border-purple-900/30">
